setTimeout(() => {
  if (!window.loaderModuleScriptName) return;
  ...
  const applyRadar = (type) => {
    loaderModule.utils.createStyles(radarStyles[type], "radarStyle");
    loaderModule.notification.show(`Radar: ${type}`, "success", 2500);
  };

  applyRadar("Oval");

  console.log("[radar.js] Oval radar applied.");
}, 250);
